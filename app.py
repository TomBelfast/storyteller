                ...
                with tab_actions:
                    ...
                    if existing_chapters:
                        ...
                        with st.expander("View Existing Chapters", expanded=False):
                            for c in existing_chapters:
                                ...
                                st.markdown(f"**{c['title']}** ({dur:.1f}s)")
                                st.text(c['content'][:200] + "...")
                                
                        gen_btn_label = "🔄 Regenerate Chapters"
                        gen_btn_key = f"regen_chapters_{story['id']}"
                    else:
                        gen_btn_label = "Generate Chapters from Outline"
                        gen_btn_key = f"gen_chapters_{story['id']}"
                    ...
                    if story.get('script_content'):
                        ...
                        edited_script = st.text_area(
                            "Full Script",
                            value=st.session_state[f"full_script_{story['id']}"],
                            height=400,
                            key=f"script_editor_{story['id']}"
                        )
                        ...
                        with col_scr1:
                            if st.button("💾 Save Full Script", key=f"save_script_{story['id']}"):
                                try:
                                    from services import update_story
                                    update_story(story['id'], script_content=edited_script)
                                    st.session_state[f"full_script_{story['id']}"] = edited_script
                                    st.success("Full script saved! Ready for Audio generation.")
                                    time.sleep(1)
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"Save failed: {e}")
                        ...
                        with col_scr2:
                            # Check if we have chapters to regenerate from
                            chapters_for_regen = db.get_chapters(story['id'])
                            if chapters_for_regen:
                                if st.button("🔄 Regenerate from Chapters", key=f"regen_script_{story['id']}", help="Re-create script from chapters"):
                                    try:
                                        from models import Chapter
                                        chapters_objs = [Chapter(**c) for c in chapters_for_regen]
                                        full_script = "\n\n".join([c.content for c in chapters_objs])
                                        from services import update_story
                                        update_story(story['id'], script_content=full_script)
                                        st.session_state[f"full_script_{story['id']}"] = full_script
                                        st.success(f"✅ Script regenerated ({len(full_script)} chars)")
                                        time.sleep(1)
                                        st.rerun()
                                    except Exception as e:
                                        st.error(f"Failed: {e}")
                            else:
                                st.caption("*No chapters to regenerate from*")

                    # --- INTERACTIVE AUDIO ---
                    st.markdown("---")
                    st.subheader("🎧 Interactive Audio (Kokoro)")
